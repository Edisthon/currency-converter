...
| **US-06** | **As a** developer, **I want to** set up a CI pipeline **so that** tests and other operations run automatically. | 5 | High | 1 | In progress | 1. Use GitHub Actions.<br>2. The GitHub Actions runs a `mvn clean test` on every push.<br>3. Pipeline should show a green indicator upon a successful test pass. |
